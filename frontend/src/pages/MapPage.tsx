...
export function MapPage(){
 ...
 useEffect(()=>{
  ...
  setMapError(null);setMetric('carbon_kg');setVisible({grids:true,buildings:true,boundary:true});
  const map=new maplibregl.Map({container:container.current,center:data.center||[127.148,35.824],zoom:11.5,attributionControl:false,style:{version:8,sources:offline?{}:{basemap:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,attribution:'© OpenStreetMap contributors'}},layers:[{id:'background',type:'background',paint:{'background-color':'#edf2ef'}},...(!offline?[{id:'basemap',type:'raster' as const,source:'basemap',paint:{'raster-opacity':0.5}}]:[])]}});
  ...
  map.on('error',e=>{if(!String(e.error.message).includes('tile.openstreetmap'))setMapError('지도 도형을 불러오지 못했습니다. 새로고침하거나 오프라인 모드를 확인해 주세요.');});
  map.on('load',()=>{
 ...
 {mapError&&<p role="alert" className="map-error">{mapError}</p>}<div className="map-source-pill">{offline?'오프라인 로컬 도형':'온라인 배경 지도'} · EPSG:5179 · {metrics[metric].unit} · 출처는 수집 데이터에서 확인</div></div></div>;
}
